...
async def nova_mensagem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.channel_post and update.channel_post.text:
        ...
        if dados['linha_odd'] and dados['confronto']:
            ...
            cursor.execute(
                'INSERT INTO mensagens (origem_msg_id, destino_msg_id) VALUES (?, ?)',
...
async def mensagem_editada(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.edited_channel_post and update.edited_channel_post.text:
        ...
        if dados['status']:
            cursor.execute(
                'SELECT destino_msg_id FROM mensagens WHERE origem_msg_id = ?',
                (msg.message_id,)
            )
...
# === EXECUTA O BOT SEM USAR asyncio.run ===
if __name__ == '__main__':
    import asyncio
    # Inicia o bot de forma assíncrona no loop de eventos ativo
    asyncio.create_task(main())  # Usa o loop de eventos já existente
    asyncio.get_event_loop().run_forever()  # Garante que o loop principal não seja fechado
